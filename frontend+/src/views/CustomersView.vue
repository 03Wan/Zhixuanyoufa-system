    ...
    <section class="page-stack fade-up">
      <AppGlassSurface as="section" class="card">
        ...
        <div class="actions" style="margin-top: 8px;">
          <button class="btn btn-primary" :disabled="saving" @click="create">{{ saving ? "保存中" : "新增客户" }}</button>
        </div>
      </AppGlassSurface>
        ...
        <table v-else class="table">
          ...
          <tbody>
            <tr v-for="c in rows" :key="c.id">
              ...
              <td>{{ c.contactPerson || "-" }} / {{ c.contactPhone || "-" }}</td>
              <td>{{ c.industry || "-" }}</td>
              <td>{{ c.targetPlatform || "-" }} / {{ c.targetMarket || "-" }}</td>
              <td>{{ c.planType || "-" }}</td>
...
const CUSTOMER_CACHE_KEY = "view-cache:customers";
const rows = ref<any[]>([]);
const form = reactive<any>({ name: "", customerType: "", contactPerson: "", contactPhone: "", industry: "", targetPlatform: "", targetMarket: "", planType: "", serviceStatus: "开通中", remark: "" });
const loading = ref(true);
const refreshing = ref(false);
const saving = ref(false);
const customerTypeOptions = ["企业客户", "中小卖家", "代运营机构", "品牌企业", "产业带机构", "服务商平台"];
...
async function load() {
  ...
  try {
    ...
    writeViewCache(CUSTOMER_CACHE_KEY, rows.value, 45_000);
  } catch (e) {
...
async function create() {
  ...
  try {
    ...
    Object.assign(form, { name: "", customerType: "", contactPerson: "", contactPhone: "", industry: "", targetPlatform: "", targetMarket: "", planType: "", serviceStatus: "开通中", remark: "" });
    await load();
  } catch (e) {
    await notify(getFriendlyError(e));
  } finally {
...
onMounted(() => {
  const cached = readViewCache<any[]>(CUSTOMER_CACHE_KEY);
  if (cached?.length) {
    rows.value = cached;
    loading.value = false;
  }
