...
import { PUBLIC_PLAN_ADD_ONS, PUBLIC_PLANS } from '@/data/publicPricing';
...
function riskByScore(score: number) {
  if (score < 50) return '严重风险';
  if (score >= 85) return '低风险';
  if (score >= 70) return '中风险';
  return '高风险';
}

function decisionByRisk(risk: string) {
  if (risk === '严重风险') return '暂缓发布';
  if (risk === '低风险') return '可发布';
  if (risk === '中风险') return '优化后发布';
  return '人工复核';
}
...
type LocalDataSeedConfig = {
  ...
  peakWindows: Array<{ startDay: number; length: number; factor: number }>;
};
...
function shuffleWithRandom<T>(list: T[], random: () => number) {
  const arr = [...list];
...
function buildPoolFromCounts<T extends string>(counts: Record<T, number>) {
  const pool: T[] = [];
  (Object.keys(counts) as T[]).forEach((key) => {
    const n = counts[key];
    for (let i = 0; i < n; i += 1) pool.push(key);
  });
  return pool;
}

function weightedPickIndex(weights: number[], random: () => number) {
...
function buildLocalDataTasks(config: LocalDataSeedConfig, timeline: TimelinePoint[], random: () => number) {
  ...
  const allocatedDayIndex: number[] = [];
...
function buildLocalDataReviews(tasks: any[], config: LocalDataSeedConfig, random: () => number) {
  ...
  return reviewCandidates.map((task, idx) => {
    ...
    const history: any[] = [{ time: submittedAt.toISOString(), action: '提交人工复核', note: '系统命中高风险规则，进入人工复核流转。' }];
    if (!isPending) {
      history.unshift({
...
function buildLocalDataLogs(
  ...
  const pushLog = (payload: any) => logs.push({ id: makeId('log'), logId: makeId('log'), ip: '127.0.0.1', result: '成功', ...payload });
...
function initLocalDataData() {
  ...
  mockPlans = PUBLIC_PLANS.map((plan) => ({ ...plan, features: { list: [...plan.features.list] } }));
  mockPlanAddOns = PUBLIC_PLAN_ADD_ONS.map((item) => ({ ...item }));
...
function resolveLocalDataRoleByEmail(email: string) {
  ...
  if (v.includes('operator')) return 'OPERATOR';
  if (v.includes('designer')) return 'DESIGNER';
...
export function setUserProfile(user: unknown) { localStorage.setItem(USER_KEY, JSON.stringify(user || {})); }
...
async function request<T>(path: string, method: RequestMethod, body?: unknown): Promise<T> {
  ...
  const raw = (await response.json().catch(() => ({}))) as Partial<ApiEnvelope<T>> & { message?: string; data?: T };
  if (!response.ok) {
  ...
  if (typeof raw.code === 'number') {
    if (raw.code !== 0) throw new ApiError(raw.message || '请求失败', response.status);
...
function toRiskText(value: unknown) {
  const v = String(value || '').toUpperCase();
  if (v === 'LOW' || String(value).includes('低')) return '低风险';
  if (v === 'MEDIUM' || String(value).includes('中')) return '中风险';
  if (v === 'HIGH' || String(value).includes('高')) return '高风险';
  if (String(value).includes('严重')) return '严重风险';
  return String(value || '-');
}

function toDecisionText(value: unknown) {
  const v = String(value || '').toUpperCase();
  if (v === 'APPROVE' || String(value).includes('可发布')) return '可发布';
  if (v === 'OPTIMIZE_AND_REVIEW' || String(value).includes('优化')) return '优化后发布';
  if (v === 'REJECT' || String(value).includes('复核')) return '人工复核';
  if (v === 'HOLD' || String(value).includes('暂缓')) return '暂缓发布';
  return String(value || '-');
}

function parsePsObjectString(text: string) {
  const raw = text.trim();
...
function normalizeUnknownObject<T = any>(value: unknown, fallback: T): T {
...
function normalizeObjectArray(value: unknown) {
  if (Array.isArray(value)) {
    return value.map((item) => {
      ...
      if (typeof item === 'string') return parsePsObjectString(item) || { value: item };
      return { value: String(item ?? '') };
    });
  }
  if (typeof value === 'string') {
    const parsed = normalizeUnknownObject<any>(value, []);
    return Array.isArray(parsed) ? parsed : [];
  }
  return [];
}

function normalizeDetectionResult(raw: any) {
  if (!raw) return null;
  const parsed = normalizeUnknownObject<any>(raw, {});
...
function normalizeReport(raw: any) {
  if (!raw) return raw;
  const content = normalizeUnknownObject<any>(raw.content, {});
  const contentTask = normalizeUnknownObject<any>(content.task, {});
...
function normalizeRule(raw: any) {
  if (!raw) return raw;
  const keywordsParsed = normalizeUnknownObject<any>(raw.keywords, raw.keywords);
...
function ensureDetection(task: any) {
...
export const api = {
  logout() {
    appendLog('用户退出', (getUserProfile() as any)?.email || '当前用户');
    clearToken();
  },
  ...
  getTaskList() {
    return run(
      async () => {
        const data = await request<any>('/tasks', 'GET');
  ...
  getTaskDetail(taskId: string) {
    return run(
      async () => normalizeTask(await request(`/tasks/${taskId}`, 'GET')),
  ...
  updateTask(taskId: string, payload: UpdateTaskPayload) {
    return run(
      () => request(`/tasks/${taskId}`, 'PATCH', payload),
      () => {
  ...
  updateTaskStatus(taskId: string, status: string) {
    return run(
      () => request(`/tasks/${taskId}/status`, 'PATCH', { status }),
      () => {
  ...
  saveTaskMaterials(taskId: string, payload: any) {
    return run(
      () => request(`/tasks/${taskId}/materials`, 'POST', payload),
      () => {
  ...
  analyzeTask(taskId: string, modelConfig?: ModelRuntimeConfig) {
    return run(
      () => request(`/tasks/${taskId}/detect`, 'POST', modelConfig || {}),
      () => {
  ...
  getDetectionResult(taskId: string) {
    return run(
      async () => normalizeDetectionResult(await request(`/tasks/${taskId}/result`, 'GET')),
      () => {
        const task = mockTasks.find((t) => t.id === taskId);
        if (!task) throw new ApiError('任务不存在');
        if (!task.detectionResult) task.detectionResult = ensureDetection(task);
        return task.detectionResult;
      },
    );
  },

  requestManualReview(taskId: string, note?: string) {
    return run(
      () => request(`/tasks/${taskId}/manual-review`, 'POST', { note }),
      () => {
        const task = mockTasks.find((t) => t.id === taskId);
        if (!task) throw new ApiError('任务不存在');
  ...
  generateReport(taskId: string) {
    return run(
      () => request(`/tasks/${taskId}/report`, 'POST'),
      async () => {
        const task = mockTasks.find((t) => t.id === taskId);
        if (!task) throw new ApiError('任务不存在');
  ...
  getReportList(params?: { page?: number; pageSize?: number; platform?: string; market?: string; riskLevel?: string; keyword?: string }) {
    return run(
      async () => {
        ...
        const data = await request<any>(path, 'GET');
        const items = Array.isArray(data) ? data : (data.items || []);
  ...
  getReportDetail(reportId: string) {
    return run(
      async () => normalizeReport(await request(`/reports/${reportId}`, 'GET')),
  ...
  deleteReport(reportId: string) {
    return run(
      () => request(`/reports/${reportId}`, 'DELETE'),
      () => {
  ...
  async downloadReport(reportId: string, format: 'pdf' | 'docx' | 'json' = 'pdf') {
    const downloadLocalDataReport = () => {
      ...
      const mimeByFormat: Record<'pdf' | 'docx' | 'json', string> = {
        pdf: 'application/pdf',
    ...
    try {
      const token = getToken();
      const detail = (await api.getReportDetail(reportId)) as any;
  ...
  getRules() {
    return run(
      async () => (await request<any[]>('/rules', 'GET')).map(normalizeRule),
      () => mockRules,
    );
  },
  ...
  createRule(payload: any) {
    return run(
      async () => normalizeRule(await request('/rules', 'POST', payload)),
      () => {
        ...
        mockRuleVersions.unshift({
          ...
          snapshot: { ...rule },
          createdAt: nowIso(),
          createdBy: (getUserProfile() as any)?.username || '当前用户',
        });
        mockRuleApprovals.unshift({
          ...
          version: rule.version || 'v1.0.0',
          submittedAt: nowIso(),
          submittedBy: (getUserProfile() as any)?.username || '当前用户',
        });
  ...
  updateRule(ruleId: string, payload: any) {
    return run(
      async () => normalizeRule(await request(`/rules/${ruleId}`, 'PATCH', payload)),
      () => {
  ...
  approveRule(approvalId: string, approved: boolean, comment?: string) {
    const item = mockRuleApprovals.find((a) => a.id === approvalId);
    if (!item) throw new ApiError('审批记录不存在');
    item.status = approved ? 'APPROVED' : 'REJECTED';
    item.comment = comment || '';
    item.reviewedAt = nowIso();
    item.reviewedBy = (getUserProfile() as any)?.username || '当前用户';
  ...
  getRuleVersions(ruleId: string) {
    return run(
      () => request(`/rules/${ruleId}/versions`, 'GET'),
      () => Promise.resolve(mockRuleVersions.filter((v) => v.ruleId === ruleId)),
    );
  },

  rollbackRuleVersion(ruleId: string, versionId: string) {
    return run(
      () => request(`/rules/${ruleId}/clone-version`, 'POST', { createdBy: (getUserProfile() as any)?.username || '当前用户', note: `rollback:${versionId}` }),
      () => {
        const target = mockRuleVersions.find((v) => v.id === versionId && v.ruleId === ruleId);
        if (!target) throw new ApiError('版本不存在');
  ...
  updateCustomerPlan(customerId: string, payload: { plan?: string; quotaTotal?: number; expireAt?: string; status?: string }) {
    const item = mockCustomers.find((c) => c.id === customerId);
    if (!item) throw new ApiError('客户不存在');
  ...
  selectSubscription(planName: string) {
    return run(
      () => request('/subscription/select', 'POST', { planName }),
      () => {
  ...
  upgradeSubscription(planName: string) {
    return run(
      () => request('/subscription/upgrade', 'POST', { planName }),
      () => api.selectSubscription(planName),
    );
  },

  quotaCheck() {
    return run(
      () => request('/tasks/quota-check', 'POST'),
      async () => {
        const usage = await api.getSubscriptionUsage() as any;
        return {
  ...
  getFiles(taskId?: string) {
    return run(
      () => request(taskId ? `/files?taskId=${encodeURIComponent(taskId)}` : '/files', 'GET'),
      () => taskId ? mockFiles.filter((f) => f.taskId === taskId) : mockFiles,
    );
  },
  ...
  createCompany(payload: any) {
    return run(
      () => request('/companies', 'POST', payload),
      () => {
  ...
  deleteCompany(companyId: string) {
    return run(
      () => request(`/companies/${companyId}`, 'DELETE'),
      () => {
        const idx = mockCompanies.findIndex((x) => x.id === companyId);
        if (idx < 0) throw new ApiError('企业不存在');
  ...
  addCompanyMember(companyId: string, userId: string, role: string) {
    return run(
      () => request(`/companies/${companyId}/members`, 'POST', { userId, role }),
      () => ({ companyId, userId, role, status: 'ACTIVE' }),
    );
  },

  getCustomersList(params?: { keyword?: string; serviceStatus?: string }) {
    return run(
      () => {
        const q = new URLSearchParams();
        if (params?.keyword) q.set('keyword', params.keyword);
        if (params?.serviceStatus) q.set('serviceStatus', params.serviceStatus);
  ...
  createCustomer(payload: any) {
    return run(
      () => request('/customers', 'POST', payload),
      () => {
  ...
  updateCustomer(customerId: string, payload: any) {
    return run(
      () => request(`/customers/${customerId}`, 'PATCH', payload),
      () => {
  ...
  getBatchTasks() {
    return run(
      () => request('/batch-tasks', 'GET'),
      () => mockBatchTasks,
    );
  },

  getBatchTaskDetail(batchId: string) {
    return run(
      () => request(`/batch-tasks/${batchId}`, 'GET'),
      () => {
        const item = mockBatchTasks.find((x) => x.id === batchId);
        if (!item) throw new ApiError('批量任务不存在');
        return item;
      },
    );
  },

  createBatchTask(payload: any) {
    return run(
      () => request('/batch-tasks', 'POST', payload),
      () => {
        const item = {
          id: makeId('batch'),
          name: payload.name,
          ...
          successCount: 0,
          failedCount: 0,
          items: (payload.items || []).map((it: any, idx: number) => ({ id: makeId('bitem'), rowNo: idx + 1, payload: it, status: 'PENDING' })),
          createdAt: nowIso(),
        };
  ...
  runBatchTask(batchId: string) {
    return run(
      () => request(`/batch-tasks/${batchId}/run`, 'POST'),
      async () => {
        const batch = mockBatchTasks.find((x) => x.id === batchId);
        if (!batch) throw new ApiError('批量任务不存在');
        ...
        for (const row of batch.items || []) {
          try {
            ...
            await api.analyzeTask((task as any).id);
            row.status = 'DONE';
            row.taskId = (task as any).id;
            success += 1;
          } catch (e: any) {
            row.status = 'FAILED';
  ...
  getMaterialVersions(taskId: string) {
    return run(
      () => request(`/tasks/${taskId}/material-versions`, 'GET'),
      () => mockMaterialVersions.filter((x) => x.taskId === taskId).sort((a, b) => b.versionNo - a.versionNo),
    );
  },

  snapshotMaterialVersion(taskId: string, payload: any) {
    return run(
      () => request(`/tasks/${taskId}/material-versions`, 'POST', payload),
      () => {
  ...
  getReportTemplates() {
    return run(
      () => request('/report-templates', 'GET'),
      () => mockReportTemplates,
    );
  },

  createReportTemplate(payload: any) {
    return run(
      () => request('/report-templates', 'POST', payload),
      () => {
        const item = {
          id: makeId('tpl'),
  ...
  updateReportTemplate(templateId: string, payload: any) {
    return run(
      () => request(`/report-templates/${templateId}`, 'PATCH', payload),
      () => {
        const item = mockReportTemplates.find((x) => x.id === templateId);
        if (!item) throw new ApiError('模板不存在');
        Object.assign(item, payload, { updatedAt: nowIso() });
        return item;
      },
    );
  ...
  deleteReportTemplate(templateId: string) {
    return run(
      () => request(`/report-templates/${templateId}`, 'DELETE'),
      () => {
        const idx = mockReportTemplates.findIndex((x) => x.id === templateId);
        if (idx < 0) throw new ApiError('模板不存在');
  ...
  getReviewTasks() {
    return run(
      async () => {
        const data = await request<any>('/reviews', 'GET');
        const items = Array.isArray(data) ? data : (data.items || []);
  ...
  startReview(reviewId: string) {
    return run(
      () => request(`/reviews/${reviewId}/start`, 'POST'),
      () => ({ success: true }),
    );
  },

  submitReviewDecision(reviewId: string, payload: { decision: '通过发布' | '退回优化' | '暂缓发布'; comment: string; reason: string }) {
    return run(
      () => request(`/reviews/${reviewId}/decision`, 'POST', payload),
      () => {
        const review = mockReviews.find((r) => r.id === reviewId || r.reviewId === reviewId);
        ...
        if (task) {
          ...
          if (task.detectionResult) task.detectionResult.decision = decisionMap[payload.decision];
        }
  ...
  getUsers() {
    return run(() => request<any[]>('/users', 'GET'), () => mockUsers);
  },

  createUser(payload: { username: string; email: string; password: string; companyName?: string; role?: string }) {
    return run(
      async () => {
        const created = await request<any>('/auth/register', 'POST', payload);
  ...
  updateUser(userId: string, payload: { role?: string; status?: string }) {
    return run(
      () => request(`/users/${userId}`, 'PATCH', payload),
      () => {
  ...
  getLogs() {
    return run(
      async () => {
        const data = await request<any>('/logs', 'GET');
  ...
  getDashboardData() {
    return run(
      ...
      () => {
        ...
        const aggregate = (list: any[], getter: (item: any) => string) => {
